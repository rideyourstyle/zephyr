...
LOG_MODULE_REGISTER(modem_ubxlib, CONFIG_MODEM_LOG_LEVEL);
...
//	}
	...
	/* modem interface */
	mdata.iface_data.hw_flow_control = NULL;
    //DT_PROP(MDM_UART_NODE,
	//					   hw_flow_control);
	mdata.iface_data.rx_rb_buf = &mdata.iface_rb_buf[0];
	mdata.iface_data.rx_rb_buf_len = sizeof(mdata.iface_rb_buf);
	ret = modem_iface_uart_init(&mctx.iface, &mdata.iface_data, NULL);
				    //MDM_UART_DEV);
	if (ret < 0) {
		goto error;
	}
